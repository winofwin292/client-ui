...
function CourseSection() {
    ...
    const getData = useCallback(async () => {
        const response = await courseApi.getAllLanding();
            ...
            <div className="py-8 px-4 mx-auto max-w-screen-xl lg:py-16 lg:px-6">
                ...
                <div className="space-y-8 lg:grid lg:grid-cols-3 sm:gap-6 xl:gap-10 lg:space-y-0">
                    {courses.map((course, index) => (
                        <div
                            key={index}
                            className="flex relative flex-col p-6 mx-auto max-w-lg text-center text-gray-900 bg-white rounded-lg border border-gray-100 shadow dark:border-gray-600 xl:p-8 dark:bg-gray-800 dark:text-white"
                        >
                            ...
                            <a
                                ...
                                className="absolute inset-x-10 bottom-6 text-white bg-indigo-600 hover:bg-indigo-700 focus:ring-4 focus:ring-indigo-200 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:text-white dark:focus:ring-indigo-900"
                            >
                                Xem chi tiết &rarr;
                            </a>
                        </div>
